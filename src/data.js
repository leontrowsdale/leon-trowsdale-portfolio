// src/data.js

export const PORTFOLIO = {
  name: "Leon Trowsdale",
  role: "Mechatronic & Robotic Engineering Student",
  cvLink: "/Leon_Trowsdale_CV.pdf",
  linkedin: "https://linkedin.com/in/leon-trowsdale",
  email: "leon.trowsdale.uk@gmail.com",
  headshot: "/Hero_Image.png", 

  skills: [
    "Fusion360", "3D Printing", "Arduino", "C", "Python", "Blender", "SketchUp", "MATLAB", "Unity", "Videography", "Music Production"
  ],

  education: [
    {
      degree: "BEng Mechatronic and Robotic Engineering",
      school: "University of Birmingham",
      year: "Sep 2024 - Jun 2027",
      description: ""
    },
    {
      degree: "A-levels",
      school: "Blenheim Sixth Form",
      year: "Sep 2024 - Jun 2027",
      description: "Physics, Maths, Further Maths, Product Design"
    }
  ],
  experience: [
    {
      role: "Engineering and Robotics Intern",
      company: "Dexory",
      year: "Jul 2026 - Jul 2027",
      description: ""
    },
    {
      role: "Lead Junior Educational Manager",
      company: "MyEdSpace",
      year: "March 2026 - Present",
      description: "Oversees team coordination, task allocation, and workflow management. Developing custom systems to track project progress, manages deadlines, while acting as a central point of communication between team members and leadership."
    },
    {
      role: "Junior Educational Manager",
      company: "MyEdSpace",
      year: "Feb 2025 - March 2026",
      description: "At MyEdSpace, I create A-level/GCSE subject content and moderate live physics lessons of 150+ students, providing real-time academic support and ensuring high-quality interactive delivery."
    },
    {
      role: "Technical Consultant",
      company: "Gillieron Scott Acoustic Design",
      year: "Jul 2023 - Present",
      description: "My work at GSAD involves modelling spaces in SketchUp, running acoustic simulations, and calculating reverberation times. I have also contributed to marketing initiatives, including automated outreach campaigns and CRM implementation"
    },
    {
      role: "Work Experience",
      company: "Cabinet Office",
      year: "Jul 2023",
      description: "Gained experience within the Cabinet Office Cyber Directorate, learning about advanced technologies used to prevent and respond to security breaches."
    }
  ],

  projects: [
    {
      slug: "automatic-dartboard",
      title: "Automatic Dartboard",
      year: "2025",
      featured: true,
      archived: false,
      category: "3D Printing / Autodarts",
      image: "/dartboard.png", 
      tools: ["3D Printing", "Electronics", "Autodarts"],
      shortDescription: "Created an automatic dartboard string system using 3D printed parts and computer vision systems.",
      details: [
        {
          type: "text",
          content: "A strong example of practical engineering testing was the development of an automated dartboard scoring system I assembled and installed in my university flat. The system used multiple cameras and controlled lighting to detect dart impacts, making structured testing essential.\n\nI began by defining performance criteria such as detection accuracy, repeatability, and robustness under varying lighting conditions. Initial tests focused on camera placement and exposure, followed by controlled trials throwing darts from consistent distances while recording errors."
        },
        {
          type: "image",
          src: "/Dartboard_Setup.png", // <--- UPDATE THIS
          caption: "Initial camera testing."
        },
        {
          type: "text",
          content: "During testing, accuracy dropped significantly, an issue that got me stumbled. By isolating variables and comparing results, I identified that the problems arose using alternative dart flights that were slightly translucent, reducing contrast and interfering with the vision system. This led to iterative improvements in lighting diffusion and detection thresholds.\n\nMechanical mounting was also tested for rigidity and alignment. Due to accommodation restrictions preventing wall fixings, I evaluated alternative methods and ultimately used thin double-sided adhesive strips at the camera bases."
        },
        {
          type: "image",
          src: "/Dartboard_Data.png", // <--- UPDATE THIS
          caption: "Initial camera testing."
        },
        {
          type: "text",
          content: "After 14,774 dart throws, only 351 corrections were required, achieving 97.62% accuracy. This project reinforced the value of controlled testing, root-cause analysis, and validating performance across realistic variations - skills directly applicable to systems integration and safety-critical validation."
        }
      ],
      pdfLink: "/Dartboard.pdf"
    },
    {
      slug: "lipo-safety-solution",
      title: "LiPo Battery Safety Solution",
      year: "2024",
      featured: true,
      archived: false,
      category: "Product Design / 3D Printing",
      image: "/lipo.png",
      tools: ["Fusion360", "3D Printing", "Blender"],
      shortDescription: "A-level Product Design NEA, portable device designed to safely contain lithium-ion battery fires during thermal runaway for drone users.",
      details: [
        {
          type: "text",
          content: "This project focuses on creating a compact and portable safety solution to reduce the dangers associated with lithium-ion battery fires, particularly those used in drone applications, after research highlighted the increasing number and severity of thermal runaway incidents.\n\nThe idea was developed in response to real-world fire cases, expert interviews, material testing, and analysis of existing commercial products that were either too expensive or not portable enough for hobbyist users."
        },
        {
          type: "image",
          src: "/Drawings.png", // <--- UPDATE THIS
          caption: "Technical drawing for the top plate."
        },
        {
          type: "text",
          content: "Through iterative sketching, CAD modelling, rapid prototyping, spring and material testing, and user feedback, the design evolved into a collapsible aluminium container lined with woven fibreglass and fitted with ventilation and flame-filtering features to safely release pressure while containing flames."
        },
        {
          type: "image",
          src: "/Renders.png", // <--- UPDATE THIS
          caption: "Technical drawing for the top plate."
        },
        {
          type: "text",
          content: "The final outcome meets the design specification for portability, usability, and fire mitigation, and the full project presentation detailing the research, development, and evaluation process is shown below."
        }
      ],
      pdfLink: "/battery-safety.pdf"
    },
    {
      slug: "pan-tilt-face-tracking",
      title: "Pan/Tilt Face Tracking",
      year: "",
      featured: true,
      archived: true,
      category: "OpenCV / Arduino",
      image: "/opencv.jpg",
      tools: ["3D Printing", "Arduino", "C", "OpenCV"],
      shortDescription: "Engineered a robust solution integrating Python and Computer Vision logic for real-time tracking.",
      details: [
        { type: "text", content: "Detailed breakdown of the Pan/Tilt Face Tracking project goes here." }
      ]
    },
    {
      slug: "3d-printed-hovercraft",
      title: "3D Printed Hovercraft",
      year: "2023",
      featured: true,
      archived: false,
      category: "3D Printing / RC",
      image: "/hovercraft_cover.png",
      tools: ["3D Printing", "Electronics"],
      shortDescription: "Engineered a robust solution integrating high-speed brushless motors and lightweight 3D printed structures.",
      details: [
        {
          type: "text",
          content: "During my A-Levels, I developed a strong interest in 3D printing and even acquired my own printer to explore its capabilities. One of the projects I’m most proud of was building a fully 3D-printed remote-control hovercraft. The idea came about after I discovered some unused drone parts - two brushless DC motors, a micro-servo, an ESC, a receiver, and lithium polymer batteries - and decided to repurpose them into something new."
        },
        {
          type: "image",
          src: "/hovercraft.jpg", // <--- UPDATE THIS
          caption: "3D printed chassis assembly."
        },
        {
          type: "text",
          content: "I printed the parts at home (which took over 15 hours), assembled the components and tested the design."
        },
        {
          type: "image",
          src: "/hovercraft_remote.jpg", // <--- UPDATE THIS
          caption: "Electronics integration and Remote."
        },
        {
          type: "text",
          content: "One key challenge I faced was the hovercraft’s skirt tearing when it moved over rough surfaces like concrete or gravel. To solve this, I experimented with materials and eventually reinforced the skirt by double-lining a bin bag, which greatly improved its durability."
        },
        {
          type: "video",
          src: "/hovercraft_back_video.mov", // <--- UPDATE THIS
          caption: "Operational Video."
        },
        {
          type: "text",
          content: "The project was both technically and creatively rewarding. It deepened my understanding of mechanical design, electronics, and problem-solving, while also demonstrating how persistence and iteration lead to tangible results. Seeing the hovercraft successfully glide across the floor was an incredibly satisfying moment - not just because it worked, but because it represented my ability to turn curiosity and resourcefulness into a real, functioning creation that inspired others around me to experiment with 3D printing too."
        }
      ]
    },
    {
      slug: "screen-reactive-backlight",
      title: "Screen Reactive Backlight",
      year: "2021",
      featured: false,
      archived: true,
      category: "Arduino / Prismatic / Hyperion",
      image: "/backlight.png",
      tools: ["Arduino", "Electronics"],
      shortDescription: "Engineered a robust solution for immersive ambient lighting using real-time screen capture processing.",
      details: [
        { type: "text", content: "Detailed breakdown of the Screen Reactive Backlight project goes here." }
      ]
    },
    {
      slug: "ubrobotics",
      title: "UBRobotics",
      year: "2025",
      featured: false,
      archived: true,
      category: "Robotics",
      image: "/ubr.jpg",
      tools: ["Fusion360", "3D Printing", "Arduino", "Electronics"],
      shortDescription: "Creative direction and technical execution of cinematic video projects and post-production.",
      details: [
        { type: "text", content: "Detailed breakdown of the Videography projects goes here." }
      ]
    },
    {
      slug: "fpv-drones",
      title: "FPV Drones",
      year: "",
      featured: false,
      archived: true,
      category: "Electronics / Betaflight",
      image: "/drone.png",
      tools: ["Electronics"],
      shortDescription: "Engineered high-performance flight systems integrating PID tuning and custom electronics layouts.",
      details: [
        { type: "text", content: "Detailed breakdown of the FPV Drones project goes here." }
      ]
    },
    {
      slug: "plywood-longboard",
      title: "Plywood Longboard",
      year: "2020",
      featured: false,
      archived: true,
      category: "Woodworking",
      image: "/longboard.jpeg",
      tools: ["Woodworking", "Hand Tools"],
      shortDescription: "Engineered a robust structural deck using vacuum-pressing and composite plywood layering.",
      details: [
        { type: "text", content: "Detailed breakdown of the Plywood Longboard project goes here." }
      ]
    },
    {
      slug: "videography",
      title: "Videography",
      year: "2023",
      featured: true,
      archived: false,
      category: "Video",
      image: "/video.png",
      tools: ["Premiere Pro", "DJI", "GoPro"],
      shortDescription: "Creative direction and technical execution of cinematic video projects and post-production.",
      details: [
        { 
        type: "text", 
        content: "I enjoy documenting and editing travel videos using Premiere Pro. Throughout this hobby I've acquired a DJI Mavic Air 2 for arial footage and Gopro Hero 9 along with my smartphone for handheld shots. I've attached one of my travel videos below." 
        },
        {
          type: "video",
          src: "/Vietnam 2024 Smaller.mp4", // <--- UPDATE THIS
          poster: "/vietnam_poster.png",
          caption: "Family Vietnam Trip - 2024"
        },
        {
        type: "text", 
        content: "I made the following video below to practice and showcase my editing skills." 
        },
        {
          type: "video",
          src: "/2049.mp4", // <--- UPDATE THIS
          poster: "/2049_poster.png",
          caption: "Blade Runner 2049 Edit"
        }
      ]
    }
  ]
};